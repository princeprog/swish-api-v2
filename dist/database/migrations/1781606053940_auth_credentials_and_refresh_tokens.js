...
async function up(db) {
    ...
    await db.schema
        ...
        .addColumn('user_id', 'uuid', (column) => column.notNull().unique().references('auth.users.id').onDelete('cascade'))
        .addColumn('password_hash', 'varchar(255)', (column) => column.notNull())
        .addColumn('password_updated_at', 'timestamptz', (column) => column.notNull().defaultTo(db.fn('now')))
    ...
    await db.schema
        ...
        .addColumn('session_id', 'uuid', (column) => column.notNull().references('auth.auth_sessions.id').onDelete('cascade'))
        .addColumn('token_hash', 'varchar(255)', (column) => column.notNull().unique())
        .addColumn('expires_at', 'timestamptz', (column) => column.notNull())
        .addColumn('revoked_at', 'timestamptz')
        .addColumn('rotated_from_token_id', 'uuid', (column) => column.references('auth.refresh_tokens.id').onDelete('set null'))
        .addColumn('created_at', 'timestamptz', (column) => column.notNull().defaultTo(db.fn('now')))
        .addColumn('updated_at', 'timestamptz', (column) => column.notNull().defaultTo(db.fn('now')))
        .execute();
    await db.schema
...
//# sourceMappingURL=1781606053940_auth_credentials_and_refresh_tokens.js.map